...
// main(sam3) 의 concat(model_ref, std::array<tensor,GGML_MAX_SRC>, dim) 시그니처와 달리
// yolov9t 는 가변 개수 tensor 를 concat 한다 → ggml_concat 순차 fold 로 동일 결과 제공.
static tensor yolo_concat(model_ref m, std::vector<tensor> ts, int dim) {
    tensor out = nullptr;
    for (tensor t : ts) {
        if (t == nullptr) continue;
        out = out ? ggml_concat(m, out, t, dim) : t;
    }
    return out;
}
...
tensor SPPELAN(model_ref m, tensor x, std::string const& name, int c1, int c2, int c3, int k, bool debug) {
    ...
    tensor cv5 = yolo_concat(m, {cv1, cv2, cv3, cv4}, 0); 
    cv5 = ggml_cont(m, cv5); 
...
DetectOutput inference(model_ref m, 
    ...
    tensor proj = ggml_new_tensor_1d(m, GGML_TYPE_F32, reg_max);
