...
tensor Conv(
    ...
    if (debug) {
        printf(
            "Weight shape: [%d,%d,%d,%d]\n", (int)weight->ne[0], (int)weight->ne[1],
            (int)weight->ne[2], (int)weight->ne[3]);
        printf(
...
tensor AConv(model_ref m, tensor x, std::string const& name, int c1, int c2, bool debug) {
    ...
    printf(
        "Before Pooling (C,W,H,N) = [%d,%d,%d,%d]\n", (int)x->ne[0], (int)x->ne[1], (int)x->ne[2],
        (int)x->ne[3]);
    x = contiguous_2d_to_whcn(m, x);
    // permute
    printf(
        "After Permute (W,H,C,N) = [%d,%d,%d,%d]\n", (int)x->ne[0], (int)x->ne[1], (int)x->ne[2],
        (int)x->ne[3]);
    tensor pooled = ggml_pool_2d(
        m, x, GGML_OP_POOL_AVG,
        /*kW*/ 2, /*kH*/ 2,
        /*sW*/ 1, /*sH*/ 1,
        /*pW*/ 0, /*pH*/ 0);

    printf(
        "After Pooling (C,W,H,N) = [%d,%d,%d,%d]\n", (int)pooled->ne[0], (int)pooled->ne[1],
        (int)pooled->ne[2], (int)pooled->ne[3]);
    pooled = whcn_to_contiguous_2d(m, pooled);
    printf(
        "After Permute (C,W,H,N) = [%d,%d,%d,%d]\n", (int)x->ne[0], (int)x->ne[1], (int)x->ne[2],
        (int)x->ne[3]);
    if (debug) {
        printf(
            "AConv Pool (C,W,H,N) = [%d,%d,%d,%d]\n", (int)pooled->ne[0], (int)pooled->ne[1],
            (int)pooled->ne[2], (int)pooled->ne[3]);
    }
    ...
    //    입력 채널 c1 → 출력 채널 c2
    tensor out = Conv(
        m, pooled, name + ".cv1.conv", c1, c2, /*k=*/3, /*stride=*/2, /*pad=*/1, /*act=*/true);
    printf("%ld, %ld, %ld, %ld\n", x->ne[0], x->ne[1], x->ne[2], x->ne[3]);
    if (debug) {
        printf(
            "AConv Out  (C,W,H,N) = [%d,%d,%d,%d]\n", (int)out->ne[0], (int)out->ne[1],
            (int)out->ne[2], (int)out->ne[3]);
    }
    return out;
}
...
*/
tensor ELAN1(
    model_ref m, tensor x, std::string const& name, int c1, int c2, int c3, int c4, bool debug) {
    // x: CWHN
    if (debug) {
        printf(
            "ELAN1 %s: x (C,W,H,N) = [%d,%d,%d,%d]\n", name.c_str(), (int)x->ne[0], (int)x->ne[1],
            (int)x->ne[2], (int)x->ne[3]);
    }
    ...
    if (!ggml_is_contiguous(split1))
        split1 = ggml_cont(m, split1);
    if (!ggml_is_contiguous(split2))
        split2 = ggml_cont(m, split2);
    ...
    if (debug) {
        printf(
            "ELAN1 %s: cat (C,W,H,N) = [%d,%d,%d,%d]\n", name.c_str(), (int)cat1234->ne[0],
            (int)cat1234->ne[1], (int)cat1234->ne[2], (int)cat1234->ne[3]);
    }
    ...
    if (debug) {
        printf(
            "ELAN1 %s: out (C,W,H,N) = [%d,%d,%d,%d]\n", name.c_str(), (int)out->ne[0],
            (int)out->ne[1], (int)out->ne[2], (int)out->ne[3]);
    }
    return out;
}
...
tensor RepConv(
    ...
    if (debug) {
        printf(
            "RepConv %s In: ne[0]=%d, ne[1]=%d, ne[2]=%d, ne[3]=%d\n", name.c_str(), (int)x->ne[0],
            (int)x->ne[1], (int)x->ne[2], (int)x->ne[3]);
    }
    ...
    if (deploy) {
        ...
        tensor conv2_out = Conv(
            m, x, cv11_name + ".conv2.conv", c1, c2, 1, s, (p - k) / 2, false,
            debug); // stride, pad=1, dilate=1
...
tensor RepNCSPELAN4(
    ...
    tensor cv1_out = Conv(
        m, x, name + ".cv1", c1, c3, /*k=*/1, /*stride=*/1, /*pad=*/0, /*bias=*/true);

    // 채널을 반으로 분할: y1 = [0:c_half), y2_input = [c_half:c3)
    tensor cv1_out_h0 = slice(
        m, cv1_out,
        /*C*/ slice_t(0, c),
        /*W*/ slice_t(), /*H*/ slice_t(), /*N*/ slice_t());
    tensor cv1_out_h1 = slice(
        m, cv1_out,
        /*C*/ slice_t(c, c3),
        /*W*/ slice_t(), /*H*/ slice_t(), /*N*/ slice_t());
...
tensor SPPELAN(
    model_ref m, tensor x, std::string const& name, int c1, int c2, int c3, int k, bool debug) {
    if (debug) {
        printf(
...
std::map<int, tensor> yolov9t_backbone(model_ref m, tensor x) {
    ...
    //        (int)features[6]->ne[0], (int)features[6]->ne[1], (int)features[6]->ne[2],
    //        (int)features[6]->ne[3]);
...
tensor dist2bbox(model_ref m, tensor distance, tensor anchor_points, bool xywh /*=true*/) {
    ...
    if (xywh) {
        ...
        tensor result = ggml_concat(m, c_xy, wh, 0);
...
DetectOutput inference(
    ...
    printf(
        "x_cat shape: [%ld,%ld,%ld, %ld]\n", x_cat->ne[0], x_cat->ne[1], x_cat->ne[2],
        x_cat->ne[3]);
    // anchors, strides shape is torch.Size([2, 8400]) torch.Size([1, 8400])
    // x_cat = ggml_transpose(m, x_cat);
    // x_cat = ggml_cont(m, x_cat);
    printf(
        "After transpose: x_cat shape: [%ld,%ld,%ld,%ld]\n", x_cat->ne[0], x_cat->ne[1],
        x_cat->ne[2], x_cat->ne[3]);
    ...
    auto [anchor_points, stride_tensor] = make_anchors(
        m, out, anchor_host, stride_host, strides_vec, 0.5f);
    ...
    // DFL (Distribution Focal Loss) layer implementation
    auto DFL = [=](model_ref m, tensor x, int c1 = 16) -> tensor {
        // x: CWHN = [C=4*c1, W=A, H=1, N=B]
        GGML_ASSERT(x->ne[0] == 4 * c1 && "DFL: C must be 4*c1");
        const int64_t A = x->ne[1], B = x->ne[3];

        // 1) [4*c1, A, 1, B] -> [c1, A, 4, B]
        tensor t = ggml_reshape_4d(m, x, /*C*/ c1, /*W*/ A, /*H*/ 4, /*N*/ B);
        if (!ggml_is_contiguous(t)) {
            t = ggml_cont(m, t);
        }

        // 2) softmax along c1 (dim0)
        tensor p = ggml_soft_max(m, t); // [c1, A, 4, B]

        // 3) proj = arange(c1) -> [c1,1,1,1] (상수)
        auto make_constant_local = [&](tensor t, const std::string& name) {
            ggml_set_name(t, name.c_str());
            ggml_set_input(t);
            ggml_set_output(t);
            return t;
        };
        auto tmp = tensor_alloc(ggml_new_tensor_1d(m, GGML_TYPE_F32, c1)); // host-backed
        auto buf = tmp.as_f32();
        for (int i = 0; i < c1; ++i) {
            buf[i] = float(i);
        }
        make_constant_local(tmp.x, "dfl.conv." + std::to_string(c1));
        tensor proj = ggml_reshape_4d(m, tmp.x, c1, 1, 1, 1);

        // 4) 기대값: sum_k p*k  (원소곱 → c1(dim0) 합)
        tensor weighted = ggml_mul(m, p, proj); // [c1, A, 4, B]
        tensor s = ggml_sum_rows(m, weighted);  // [1,  A, 4, B]

        // 5) [1,A,4,B] -> [4,A,1,B] (CWHN에서 C=4)
        tensor out = ggml_permute(m, s, 2, 1, 0, 3); // (1,A,4,B) -> (4,A,1,B)
    ...
    ggml_set_name(dfl_output, "dist2bbox_out"); // 디버깅에 유용 !TODO
    // tensor dfl_output = dfl_forward(m, box, reg_max, proj, false);
    // printf("dfl_output shape: [%ld,%ld,%ld,%ld]\n", dfl_output->ne[0], dfl_output->ne[1],
    // dfl_output->ne[2], dfl_output->ne[3]); Decode bounding boxes
    tensor dbox = dist2bbox(m, dfl_output, anchor_points, true);
    printf("dbox shape: [%ld,%ld,%ld,%ld]\n", dbox->ne[0], dbox->ne[1], dbox->ne[2], dbox->ne[3]);
    // Multiply by strides
    tensor strides_rep = ggml_repeat(m, stride_tensor, dbox); // [4,8400,1,1]
    strides_rep = ggml_cont(m, strides_rep);
    dbox = ggml_mul(m, dbox, strides_rep);
    ...
    ggml_set_output(out.predictions_bbox);
    out.predictions_cls = std::move(cls);
    ggml_set_output(out.predictions_cls);
    out.dist2bbox = std::move(dfl_output);
    ggml_set_output(out.dist2bbox);
    // expose constants so caller can upload before compute and set host buffers
    out.anchor_points = anchor_points;
    ggml_set_output(out.anchor_points);
    out.strides_points = stride_tensor;
    ggml_set_output(out.strides_points);
    out.reg_max = reg_max;
    ggml_set_output(out.strides_points);
    out.dfl_proj_host_data.resize(reg_max);
    for (int i = 0; i < reg_max; ++i)
        out.dfl_proj_host_data[i] = float(i);
...
DetectOutput detect_forward(
    ...
    auto head = [=](model_ref m, tensor feature, std::string name, int c1, int c2, int c3,
                    int pad = 0) {
    ...
    for (size_t i = 0; i < ch.size(); i++) {
        ...
        tensor reg_head = head(
            m, features[i], reg_base + "." + std::to_string(i), ch[i], c2, reg_max * 4);
        cv2.push_back(reg_head);
        tensor cls_head = head(
            m, features[i], cls_base + "." + std::to_string(i), ch[i], c3, nc, 1);
        cv3.push_back(cls_head);
    }
    for (size_t i = 0; i < ch.size(); i++) {
        printf(
            "cv2[%zu] shape: [%ld,%ld,%ld,%ld]\n", i, cv2[i]->ne[0], cv2[i]->ne[1], cv2[i]->ne[2],
            cv2[i]->ne[3]);
        printf(
            "cv3[%zu] shape: [%ld,%ld,%ld,%ld]\n", i, cv3[i]->ne[0], cv3[i]->ne[1], cv3[i]->ne[2],
            cv3[i]->ne[3]);
        tensor concat_out = concat(m, {cv2[i], cv3[i]}, 0);
        tensor reshaped_combined = ggml_reshape_2d(
            m, concat_out, concat_out->ne[0], concat_out->ne[1] * concat_out->ne[2]);
        reshaped_combined = ggml_cont(m, reshaped_combined);
        ggml_set_output(reshaped_combined);
        out.features.push_back(reshaped_combined);
        ggml_set_output(concat_out);
        out.raw_outputs.push_back(concat_out);
    }
...
std::pair<tensor, tensor> make_anchors(
    ...
    // 반환 전에 host 버퍼를 채워 상위에서 업로드할 수 있도록 함께 반환자 외부에서도 보유
...
def check_img_size(imgsz, s=32, floor=0):
    ...
    if new_size != imgsz:
        LOGGER.warning(f'WARNING ⚠️ --img-size {imgsz} must be multiple of max stride {s}, updating
to {new_size}') return new_size
*/
...
int check_img_size(int imgsz, int s, int floor) {
    ...
    if (new_size != imgsz) {
        printf(
            "WARNING ⚠️ --img-size %d must be multiple of max stride %d, updating to %d\n", imgsz, s,
            new_size);
    }
    return new_size;
}
...
void resize_rgb_bilinear(uint8_t* src_data, i32x2 src_extent, uint8_t* dst_data, i32x2 dst_extent) {
    ...
    for (int y = 0; y < dst_extent[1]; ++y) {
        for (int x = 0; x < dst_extent[0]; ++x) {
            ...
            // 3채널 동시 처리
            for (int c = 0; c < 3; ++c) {
                double value = src_data[(y0 * src_extent[0] + x0) * 3 + c] * w00 +
...
image_data letterbox(
    ...
    // 스케일 비율 (new / old)
    float r = std::min(
        static_cast<float>(new_shape[0]) / shape[0], static_cast<float>(new_shape[1]) / shape[1]);
    ...
    // 패딩 계산
    i32x2 new_unpad = {
        static_cast<int>(round(shape[1] * r)), static_cast<int>(round(shape[0] * r))};
...
std::vector<int> nms(
    ...
    for (size_t i = 0; i < order.size(); ++i) {
        ...
        for (size_t j = i + 1; j < order.size(); ++j) {
            ...
            float b2[4] = {
                boxes[idx_j][0] + cj, boxes[idx_j][1], boxes[idx_j][2] + cj, boxes[idx_j][3]};
...
def non_max_suppression(
    ...
    # Checks
    assert 0 <= conf_thres <= 1, f"Invalid Confidence threshold {conf_thres}, valid values are
between 0.0 and 1.0" assert 0 <= iou_thres <= 1, f"Invalid IoU {iou_thres}, valid values are between
0.0 and 1.0" if isinstance(prediction, (list, tuple)):  # YOLOv8 model in validation model, output =
(inference_out, loss_out) prediction = prediction[0]  # select only inference output
...
to track idxs
    ...
    for xi, (x, xk) in enumerate(zip(prediction, xinds)):  # image index, (preds, preds indices)
        ...
        if n > max_nms:  # excess boxes
            filt = x[:, 4].argsort(descending=True)[:max_nms]  # sort by confidence and remove
excess boxes x, xk = x[filt], xk[filt] # Batched NMS c = x[:, 5:6] * max_wh  # classes scores = x[:,
4]  # scores boxes = x[:, :4] + c  # boxes (offset by class) i = torchvision.ops.nms(boxes, scores,
iou_thres)  # NMS i = i[:max_det]  # limit detections
...
void scale_boxes(
    ...
    float gain = std::min(
        (float)model_shape[0] / img_shape[0], (float)model_shape[1] / img_shape[1]);
...
void draw_detections(
    ...
    if (img.format == image_format::rgb_u8 || img.format == image_format::rgba_u8) {
        ...
        image_data temp_u8 = image_f32_to_u8(
            img, channels == 4 ? image_format::rgba_u8 : image_format::rgb_u8);
        ...
        for (auto const& det : detections) {
            ...
            const char* class_name =
                (det.class_id >= 0 && (size_t)det.class_id < class_names.size())
            ...
            printf(
                "Detection: %s %s at [%d, %d, %d, %d] (obj_conf: %.2f, cls_conf: %.2f)\n",
                class_name, conf_str, x1, y1, x2, y2, det.confidence, det.class_confidence);
            ...
            draw_filled_rectangle(
                img_data, width, height, channels, x1, label_y, x1 + text_width,
                label_y + text_height, r, g, b);
            draw_text(
                img_data, width, height, channels, x1 + 2, label_y + 2, conf_str, 255, 255, 255);
        }
    ...
    for (auto const& det : detections) {
        ...
        printf(
            "Detection: %s %s at [%d, %d, %d, %d] (obj_conf: %.2f, cls_conf: %.2f)\n", class_name,
            conf_str, x1, y1, x2, y2, det.confidence, det.class_confidence);
        ...
        draw_filled_rectangle(
            img_data, width, height, channels, x1, label_y, x1 + text_width, label_y + text_height,
            r, g, b);
        draw_text(img_data, width, height, channels, x1 + 2, label_y + 2, conf_str, 255, 255, 255);
    }
}
} // namespace visp::yolov9t
...
image_data yolov9t_process_input(image_data image, yolov9t_params const& p) {
    ...
    image_data lb = letterbox(
        std::move(image), {p.input_size, p.input_size}, u8x3{114, 114, 114}, true, false, true,
...
static void write_tensor_txt(tensor t, char const* filename) {
    ...
    fprintf(
        ...
        (long)t->ne[3], ggml_type_name(t->type));
...
void save_input_to_txt(tensor input, char const* filepath) {
    ...
    fprintf(
        f, "# input shape C,H,W,N = %ld,%ld,%ld,%ld\n# type = %s\n", (long)input->ne[0],
        (long)input->ne[1], (long)input->ne[2], (long)input->ne[3], ggml_type_name(input->type));
