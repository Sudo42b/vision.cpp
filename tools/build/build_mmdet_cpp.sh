...
#                  ⚠️ **어떤 라이브러리로 빌드했는지는 명령줄에 남아야 한다.** env 로만 받으면
#                     낡은 build/ 를 참조해 링크가 깨져도 로그만 봐서는 알 수 없다.
#                     `VISP_BUILD` 도 계속 읽지만 인자가 우선이다.
#
set -e
...
# `--build <dir>` 는 어디에 두든 받는다 — 위치 인자와 섞이지 않게 먼저 걷어낸다.
ARG_BUILD=""
POS=()
while [ $# -gt 0 ]; do
  case "$1" in
...
GEN="${1:?usage: build_mmdet_cpp.sh [--build <dir>] <gen_dir> [arch_name]}"
GEN="$(cd "$GEN" && pwd)"
ARCH="${2:-}"
if [ -z "$ARCH" ]; then
