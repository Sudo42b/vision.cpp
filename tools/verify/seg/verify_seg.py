...
import mmseg_wrap
SZ = %(SZ)d
m, shapes = mmseg_wrap.build("%(CFG)s", "%(CK)s", SZ)
torch.save(m, "seg.pt")
...
def verify(fam, cfg, weights):
    ...
    for stale in ("seg.pt", "ref.shapes.txt"):
        try:
    ...
    open(os.path.join(d, "ref.py"), "w").write(
        REF % {"FE": FE, "FE_DET": FE_DET, "SZ": SZ, "CFG": cfg, "CK": ck})
...
''' % (SZ, SZ)], d, {"PYTHONPATH": f"{FE}:{FE_DET}:{P}:{GGUF_PY}"})
    ...
    x = run([os.path.join(gen, "run_dump"), os.path.join(gen, "Seg.gguf"),
             os.path.join(d, "in.bin"), os.path.join(d, "cpp"), str(SZ)], d)
    ...
    note = f"L1 {worst_l1:.2e} · L2 {worst_l2:.2e} · 출력 {len(shapes)}"
