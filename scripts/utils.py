...
def is_ascii(s=''):
    # Is string composed of all ASCII characters? (note: non-ASCII strings cannot be used with cv2)
    return len(s) == len(s.encode(encoding='ascii', errors='ignore'))
...
def smart_inference_mode(torch_1_9=torch.__version__ >= '1.9.0'):
...
def select_device(device='', batch_size=0):
    ...
    if device:  # non-cpu device requested
