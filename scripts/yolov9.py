...
import torch
from yolo_module import create_yolov9t
...
class YOLOv9t_Seq(nn.Module):
    ...
    def __init__(self, ch=3, nc=80):
        ...
        self.model = nn.Sequential(
            ...
            AConv(64, 48),                   # 16
            Concat(1),                       # 17
            RepNCSPELAN4(144, 96, 96, 48, 3), # 18 - N4 output
            AConv(96, 64),                   # 19
            Concat(1),                       # 20
            RepNCSPELAN4(192, 128, 128, 64, 3), # 21 - N5 output
        )
    ...
    def load_ultralytics_weights_fixed(self, weight_path):
        ...
        print("\nWeights loaded!")
def advanced_weight_transfer(source_path, target_model):
    """
    Advanced weight transfer with layer-by-layer matching
    
    Args:
        source_path (str): Path to source weights (.pt file)
        target_model (nn.Module): Target model to load weights into
    """
    ...
    for target_key, target_param in target_state.items():
        ...
        for source_key, source_param in source_state.items():
            ...
            if target_param.shape == source_param.shape:
                ...
                # Simple matching: check if last few parts are the same
                if target_parts[-2:] == source_parts[-2:]:  # Match last 2 parts
                    target_state[target_key] = source_param.clone()
                    transferred += 1
                    matched = True
                    break
    ...
    print(f"\nTransferred: {transferred}/{len(target_state)} parameters")
    print(f"Failed: {len(failed)} parameters")
    ...
    return target_model
def debug_key_structure(weight_path):
    """
    Debug utility to understand the structure of source weights
    
    Args:
        weight_path (str): Path to .pt file
    """
    print(f"Analyzing {weight_path}...\n")
    ...
    # Print structure
    print("=== Source Model Layer Structure ===\n")
    for layer_num in sorted(layers.keys(), key=lambda x: int(x) if x.isdigit() else 999):
        print(f"Layer {layer_num}: {len(layers[layer_num])} parameters")
        
        # Show unique parameter patterns
        patterns = set()
        for key in layers[layer_num]:
            # Extract pattern (e.g., cv1.conv, cv2.bn, etc.)
            parts = key.split('.')[2:]  # Skip 'model.X'
            if len(parts) >= 2:
        ...
        print(f"  Examples:")
        for key in sorted(layers[layer_num])[:3]:
            print(f"    {key}")
        print()


def create_manual_mapping(source_path, target_model):
    """
    Create manual mapping by inspecting both models
    
    Args:
        source_path (str): Path to source weights
        target_model (nn.Module): Target model
        
    Returns:
        dict: Manual key mapping
    """
    # Load source
    checkpoint = torch.load(source_path, map_location='cpu')
    if isinstance(checkpoint, dict) and 'model' in checkpoint:
    ...
    # Manual mapping rules based on YOLOv9 structure
    mapping = {}
    
    # Common layer mapping patterns
    layer_mappings = {
        # ELAN layers: out -> cv4
        'out': 'cv4',
    }
    ...
    for target_key in target_state.keys():
        ...
        # Rule 1: Replace 'out' with 'cv4' for ELAN modules
        if '.out.' in source_key:
            source_key = source_key.replace('.out.', '.cv4.')
        
        # Rule 2: Detect head mapping
        if source_key.startswith('detect.'):
            source_key = source_key.replace('detect.', 'model.22.')
        
        # Check if transformed key exists
    ...
    return mapping
# 먼저 소스 모델의 정확한 구조를 확인하는 함수
def analyze_module_structure(weight_path):
    """Analyze the exact module structure from source weights"""
    checkpoint = torch.load(weight_path, map_location='cpu', weights_only=False)
    ...
    layers_to_check = [2, 4, 6, 8, 12, 15, 18, 21]
    
    print("=== Module Structure Analysis ===\n")
    for layer_idx in layers_to_check:
        print(f"Layer {layer_idx}:")
        layer_keys = [k for k in state_dict.keys() if k.startswith(f'model.{layer_idx}.')]
        ...
        for key in layer_keys:
            parts = key.split('.')
            if len(parts) >= 3:
                submodule = parts[2]  # cv1, cv2, cv3, cv4, etc.
                if submodule not in submodules:
                    submodules[submodule] = []
                submodules[submodule].append(key)
        
        for submodule, keys in sorted(submodules.items()):
            # Find conv weight to check dimensions
            conv_keys = [k for k in keys if 'conv.weight' in k and 'bn' not in k]
            if conv_keys:
                weight = state_dict[conv_keys[0]]
                print(f"  {submodule}: {weight.shape} (out, in, h, w)")
        print()

# 실행해서 구조 확인
# analyze_module_structure('yolov9t.pt')
# load_yolov9t.py 파일 수정
def load_yolov9t():
