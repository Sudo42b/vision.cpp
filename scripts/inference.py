...
def parse_opt():
    ...
    parser.add_argument(
        "--weights", nargs="+", type=str, default="yolov9t_converted.pth", help="model path"
    )
    parser.add_argument(
        "--source", type=str, default="../tests/input/cat-and-hat.jpg", help="image path"
    )
    parser.add_argument(
        "--data", type=str, default="../tests/input/coco.yaml", help="model yaml (for class names)"
    )
    parser.add_argument(
        "--imgsz", "--img", "--img-size", nargs="+", type=int, default=[640], help="h w"
    )
...
if __name__ == "__main__":
    ...
    # Draw boxes
    annotator = Annotator(
        im0.copy(), line_width=opt.line_thickness, example=str(list(names.values()))
    )
